...
- [X] 6x5 격자를 통해서 5글자 단어를 6번 만에 추측한다.
  ...
  - [X] 입력받은 단어의 길이는 5자이다.
  - [X] 마지막 실행은 실행횟수/전체 실행 가능 횟수가 현출된다 예) 6/6
...
- [X] 플레이어가 답안을 제출하면 프로그램이 정답과 제출된 단어의 각 알파벳 종류와 위치를 비교해 판별한다.
  - [X] 입력 문자와 정답문자 알파벳과 위치가 동일하면 MATCH
  - [X] 입력 문자와 정답문자 알파벳가 동일하지만 위치가 동일하지 않으면 EXIST
  - [X] 입력 문자와 정답문자 알파벳과 위치가 동일하지 않으면 NON_EXIST
